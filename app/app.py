...
def load_data():
    ...
    tto = None
    ttp_parquet = DATA_DIR / "tt_by_origin.parquet"
    ttp_csv = DATA_DIR / "tt_by_origin.csv"

    if ttp_parquet.exists():
        try:
            tto = pd.read_parquet(ttp_parquet)
        except Exception as e:
            st.warning(f"Could not read Parquet: {e}")

    if tto is None and ttp_csv.exists():
        try:
            tto = pd.read_csv(ttp_csv)
        except Exception as e:
