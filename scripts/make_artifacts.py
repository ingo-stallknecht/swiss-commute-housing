...
def main():
    ...
    station_choices = (
        stops_for_lookup[["station_id", "stop_name"]]
        .dropna()
        .drop_duplicates()
        .sort_values("stop_name")
    )
    station_choices = station_choices[
        ~station_choices["stop_name"].str.contains("Bahn-2000", case=False, na=False)
    ]
    ...
    by_station = (
        st_times.groupby("station_id", as_index=False)["stop_id"]
        .count()
        .rename(columns={"stop_id": "events"})
        .sort_values("events", ascending=False)
    )
    top20 = by_station.head(20).copy()
    name_map = stops_for_lookup.groupby("station_id")["stop_name"].agg(
    ...
    if extra_rows:
        top20 = pd.concat(
            [top20[["station_id", "origin_name"]], pd.DataFrame(extra_rows)], ignore_index=True
        )
    top20 = (
        top20.dropna(subset=["origin_name"]).drop_duplicates("station_id").reset_index(drop=True)
    )
    ...
    if rows:
        ...
        tt_by_origin["avg_travel_min"] = pd.to_numeric(
            tt_by_origin["avg_travel_min"], errors="coerce"
        )
        tt_by_origin.to_parquet(ART / "tt_by_origin.parquet", index=False)
        print(
            f"  → wrote {ART/'tt_by_origin.parquet'} ({len(tt_by_origin):,} rows, "
    ...
    meta_out = {"origin_station": ORIGIN_STATION, **meta}
