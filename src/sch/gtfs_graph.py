...
def build_station_graph(
    ...
    st["station_id"] = st["stop_id"].map(
        stops.set_index("stop_id")["station_id"].to_dict()
    )
    ...
    def _sanitize(edges_df, stxy_dict, min_sec=20.0, vmax_kmh=160.0):
        u_xy = edges_df["station_id"].map(stxy_dict); v_xy = edges_df["next_station"].map(stxy_dict)
    ...
    for r in edge_sane.itertuples(index=False):
        u = str(r.station_id); v = str(r.next_station); w = float(r.w_sec)
...
def compute_commute_minutes_from(origin_name: str, g_polys: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    def _ensure_lv95(gdf):
        if gdf.crs is None: gdf = gdf.set_crs(CRS_CH)
        if str(gdf.crs).lower() != "epsg:2056": gdf = gdf.to_crs(CRS_CH)
        return gdf
    ...
    for k in range(K_NEAREST_ST):
        ...
        cols = {}
        if "GEMEINDE_CODE_left" in kn.columns: cols["GEMEINDE_CODE_left"] = "GEMEINDE_CODE"
        if "time_sec_station_right" in kn.columns: cols["time_sec_station_right"] = f"time_sec_{k}"
        if "station_id_right" in kn.columns: cols["station_id_right"] = f"sid_{k}"
